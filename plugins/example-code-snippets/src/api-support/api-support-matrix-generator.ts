...
function renderApiGroups(
  ...
  return nodes;
  //   output += `| Feature                  | ${SDKS.map(s =>
  //     sdkDisplayName(s.sdk).padEnd(18, ' ')
  //   ).join(' | ')} |`;
  //   output += `|--------------------------|-${SDKS.map(
  //     () => '------------------'
  //   ).join('-|-')}-|`;
  ...
  //     const displayName = typeof api === 'string' ? api : api.displayName;
  //     output += `| ${displayName.padEnd(24, ' ')} | ${SDKS.map(sdk => {
  //       if (
  //         allSdksApiSupport.get(sdk.sdk.valueOf())?.get(displayName) === true
  //       ) {
